...
- `data::Matrix{Float64}`
"""
function conv(width::Int, height::Int, data::Matrix{Float64}, filter::Matrix{Float64})::Matrix{Float64}
    padded::Matrix{Float64} = zeros(Float64, height + 2, width + 2)
    padded[2:end-1, 2:end-1].=data

    for i in 1:height
        for j in 1:width
            data[i, j] = sum(padded[i:i+2, j:j+2] .* filter)
        end
    end
...
- `data::Matrix{Int16}`
"""
function conv(width::Int, height::Int, data::Matrix{Int16}, filter::Matrix{Int16})::Matrix{Int16}
...
```julia
  clipping(width::Int, height::Int,
           data::Matrix{Float64})::Matrix{Int16}
```
...
- `img::Matrix{Int16}`
"""
function clipping(width::Int, height::Int, data::Matrix{Float64})::Matrix{Int16}
...
```julia
  clipping(width::Int, height::Int,
           data::Matrix{Int16})::Matrix{Int16}
```
...
- `img::Matrix{Int16}`
"""
function clipping(width::Int, height::Int, data::Matrix{Int16})::Matrix{Int16}
    for i in 1:height
        for j in 1:width
            if data[i, j] < 0
                data[i, j] = 0
...
- `img::Matrix{Float64}`
"""
function sobelLR(width::Int, height::Int, data::Matrix{Float64})::Matrix{Float64}
...
function sobelLR(width::Int, height::Int, data::Matrix{Int16})::Matrix{Int16}
    filterLR::Matrix{Int16} = [-1 0 1;
                               -2 0 2;
                               -1 0 1]
    imgLR::Matrix{Int16} = conv(width, height, data, filterLR)
...
- `img::Matrix{Float64}`
"""
function sobelTD(width::Int, height::Int, data::Matrix{Float64})::Matrix{Float64}
...
function sobelTD(width::Int, height::Int, data::Matrix{Int16})::Matrix{Int16}
    filterTD::Matrix{Int16} = [  1  2  1;
                                 0  0  0;
                                -1 -2 -1]
    imgTD::Matrix{Int16} = conv(width, height, data, filterTD)
...
function laplacian(width::Int, height::Int, data::Matrix{Int16})::Matrix{Int16}
    filter::Matrix{Int16} = [0  1 0;
                             1 -4 1;
                             0  1 0]
    img::Matrix{Int16} = conv(width, height, data, filter)
    result::Matrix{Int16} = clipping(width, height, img)
    return result
end
...
- `img::Matrix{Int16}`
"""
function unsharpMask(width::Int, height::Int, data::Matrix{Int16}, k::Int8)::Matrix{Int16}
    img::Matrix{Float64} = data
